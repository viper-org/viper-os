...
namespace echis
{
    ...
    void framebufferImpl::Init(void* base, uint64_t pitch, uint64_t horiz, uint64_t vert)
    {
        screen.base = base;
        screen.pitch = pitch;
        screen.horiz = horiz;
    ...
    void framebufferImpl::PutPixel(uint32_t x, uint32_t y, uint32_t color)
    {
    ...
    void framebufferImpl::DrawBitmap(void* bitmap, uint32_t x, uint32_t y, uint32_t sizeX, uint32_t sizeY, uint32_t foreground, uint32_t background)
    {
        uint8_t* address = reinterpret_cast<uint8_t*>(screen.base);
        uint32_t index = x * 4 + y * screen.pitch;
